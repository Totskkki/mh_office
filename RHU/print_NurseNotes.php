...
if (isset($_GET['id'])) {
    $recordId = $_GET['id'];
        ...
        GROUP BY h.date, h.time, h.nureNotes";
    ...
    if ($records) {
        ...
        $pdf->SetTitle('NURSE\'S NOTES');
        $pdf->SetSubject('NURSE\'S Notes Report');
        ...
        $html = '<h2 style="text-align: center;">NURSE\'S NOTES</h2>';
        ...
        foreach ($records as $record) {
            $html .= '<tr>
                       ...
                       <td colspan="2">' . nl2br(htmlspecialchars($record['nureNotes'])) . '</td>
                     </tr>';
        }
        ...
        $pdf->Output('Nurses_notes.pdf', 'I');
    } else {
        echo "No nurse's notes found for this patient!";
    }
} else {
