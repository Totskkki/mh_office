...
<body>
    ...
    <div class="page-wrapper">
        ...
        <div class="app-container">
            ...
            <div class="app-header d-flex align-items-center ">
                <a href="prenatal.php" class="btn btn-primary">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <?php if (isset($patientData)) : ?>
                                    <h3 class="card-title "> <?php echo htmlspecialchars(ucwords($patientData['name'])); ?>
                                        <span>
                                            <button type="button" id="referButton"  data-discharged-id="<?php echo htmlspecialchars($patientData['patientID']); ?>" class="btn btn-warning float-end" data-bs-toggle="modal" data-bs-target="#referModal">
                                                <i class="icon-arrow-right"></i> Refer Patient </button>
                                        </span>
                                    </h3>
                            </div>
                            ...
                            <!-- Refer Modal Structure -->
                            <div class="modal fade" id="referModal" tabindex="-1" aria-labelledby="referModalLabel" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="referModalLabel">Confirm Referral</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body">
                                            Are you sure you want to refer this patient?
                                        </div>
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">No</button>
                                            <button type="button" class="btn btn-primary" id="confirmRefer">Yes</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <script>
       document.getElementById('confirmRefer').addEventListener('click', function() {
    // Get the patient ID from the button's data attribute
    var patientId = document.getElementById('referButton').getAttribute('data-discharged-id');
    var complaintId = document.getElementById('hidden_id1').value; 
    console.log(patientId, complaintId);
        ...
        .then(data => {
            if (data.success) {
                ...
                window.location.href = "print_referral.php?id=" + patientId;
            } else {
        ...
        // Hide the modal
        var referModal = bootstrap.Modal.getInstance(document.getElementById('referModal'));
        referModal.hide();
    });
    ...
    checkboxes.forEach((item) => {
        if (item !== checkbox) {
            item.checked = false; // Uncheck the other checkbox
        }
    });
}
    </script>
