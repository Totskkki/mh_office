...
// Define required fields for form data
$requiredFields = [
    'userID', 'first_name', 'middlename', 'lastname', 'email',
...
// Initialize profile picture path to null
$profilePicturePath = null;

// Check for profile picture upload
if (isset($_FILES['profile_picture']) && is_uploaded_file($_FILES['profile_picture']['tmp_name'])) {
    $uploadDir = __DIR__ . '/../user_images/';  // Ensure correct absolute path
    if (!is_dir($uploadDir)) {
        mkdir($uploadDir, 0755, true);  // Ensure the directory exists with proper permissions
    }
    ...
    $profilePicturePath = 'user_images/' . $fileName;  // Store the relative path for DB
}

try {
    // Update personnel data (first_name, middle_name, etc.)
    ...
    // Update position data (specialty, professional type, license number)
